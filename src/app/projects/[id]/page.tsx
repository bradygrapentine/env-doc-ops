      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <section className="lg:col-span-2 bg-white border rounded p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            Required CSV columns: <code className="bg-gray-100 px-1 rounded">intersection,period,approach,inbound,outbound,total</code>.
            <code className="bg-gray-100 px-1 rounded ml-1">period</code> must be AM, PM, MIDDAY, or OTHER.
          </p>
              ...
              <div className="overflow-x-auto rounded border">
                <table className="min-w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      {["Intersection","Period","Approach","Inbound","Outbound","Total"].map(h => (
                ...
                {rows.length > 50 && (
                  <div className="text-xs text-gray-500 px-3 py-2 bg-gray-50">Showing first 50 of {rows.length}.</div>
                )}
              </div>
            </div>
          )}
        ...
        <aside className="bg-white border rounded p-6 h-fit">
          ...
          <dl className="text-sm space-y-2">
            ...
            <Row label="Peak AM" value={metrics.highestAmIntersection ? `${metrics.highestAmIntersection} (${metrics.highestAmTotal})` : "—"} />
            <Row label="Peak PM" value={metrics.highestPmIntersection ? `${metrics.highestPmIntersection} (${metrics.highestPmTotal})` : "—"} />
          </dl>
        </aside>
      </div>
    </div>
