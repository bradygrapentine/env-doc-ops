...
export default function EditProjectPage({ params }: { params: { id: string } }) {
  ...
  if (!project) return <div className="text-sm text-red-600">{error ?? "Failed to load project"}</div>;

  return (
    <div className="max-w-xl">
      <div className="mb-6">
        <Link href={`/projects/${params.id}`} className="text-sm text-gray-500 hover:underline">← Back to project</Link>
      ...
      <form onSubmit={onSubmit} className="space-y-4 bg-white border rounded p-6">
        ...
        <Field name="jurisdiction" label="Jurisdiction" required defaultValue={project.jurisdiction} />
        ...
        <Field name="developmentSummary" label="Development summary" required textarea defaultValue={project.developmentSummary} />
...
function Field({
  name, label, required, defaultValue, textarea,
